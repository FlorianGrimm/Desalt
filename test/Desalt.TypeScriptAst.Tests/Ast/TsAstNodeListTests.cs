...
namespace Desalt.TypeScriptAst.Tests.Ast
{
    ...
    using Factory = Desalt.TypeScriptAst.Ast.TsAstFactory;
    ...
    public class TsAstNodeListTests
    {
        ...
        {
            var list = TsAstNodeList.Create<ITsArrayElement>();
        ...
        {
            var list = TsAstNodeList.Create(Factory.True);
        ...
        {
            var list = TsAstNodeList.Create(Factory.Number(1), Factory.Number(2));
        ...
        {
            var list = TsAstNodeList.CreateRange(new[] { Factory.True, Factory.False });
        ...
        {
            var list = TsAstNodeList.Create(Factory.False, Factory.True, Factory.False);
        ...
        {
            var list = TsAstNodeList.Create(Factory.True, Factory.False);
        ...
        public void Count_should_only_count_the_nodes_and_not_the_tokens()
        {
            for (int i = 1; i <= 5; i++)
            {
                var list = TsAstNodeList.CreateRange(Enumerable.Range(1, i).Select(_ => Factory.True));
        ...
        {
            var list = TsAstNodeList.Create(Factory.True, Factory.False, Factory.True);
        ...
        [Test]
        public void Emit_should_produce_an_empty_string_for_an_empty_list()
        {
            var list = TsAstNodeList<ITsExpression>.Empty;
            list.EmitAsString().Should().BeSameAs(string.Empty);
        }

        [Test]
        public void Emit_should_print_a_single_node()
        {
            var list = TsAstNodeList.Create(Factory.Null);
        ...
        {
            var list = TsAstNodeList.Create(Factory.AnyType, Factory.StringType, Factory.BooleanType);
...
#pragma warning restore CS1718 // Comparison made to same variable
        ...
        {
            var list1 = TsAstNodeList.Create(Factory.Null);
            var list2 = TsAstNodeList.Create(Factory.Null);
